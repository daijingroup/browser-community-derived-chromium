...
| HTTPS upgrade exceptions | `browser-kitech-derived-chromium` `patches/0009-kiryra-integrate-https-upgrade-exceptions.patch`: `https_upgrade_exceptions_core.h/cc` adapted (Kiryra path layout), copied into `components/https_upgrade_exceptions/core`. `kiryra::CanUpgradeToHTTPS()` checks a bundled exception list (ASUS/TP-Link/Xiaomi local router admin UIs) inside `HttpsUpgradesInterceptor::MaybeCreateLoaderOnHstsQueryCompleted`, after the enterprise allowlist check. Always-on safety valve; no user pref. |
